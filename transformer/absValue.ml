  ...
  struct 
    ...
    module CompOp = struct
      ...
      let (>) n1 n2 = 
        ...
        | IntTop, _ | _, IntBot -> IntBot
        | IntSet s1, IntSet s2 -> 
            if (S.min_elt s1) > (S.max_elt s2) then alpha (Z.of_int 1) else alpha (Z.of_int 0)

      let (>=) n1 n2 = 
        match n1, n2 with
        | IntBot, _ | _, IntTop -> IntTop
        | IntTop, _ | _, IntBot -> IntBot
        | IntSet s1, IntSet s2 -> if (S.min_elt s1) >= (S.max_elt s2) then alpha (Z.of_int 1) else alpha (Z.of_int 0)
